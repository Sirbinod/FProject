...
class UserDatabase {
  ...
  Future _onCreate(Database db, int version) async {
    await db.execute(
      "CREATE TABLE users(id INTEGER PRIMARY KEY autoincrement, name TEXT, password TEXT, mobile TEXT)",
    );
    await db.execute(
      "CREATE TABLE logins(name TEXT, mobile TEXT,password TEXT)",
    );
  }
  ...
  Future<User> checkUserLogin(String mobile, String password) async {
    ...
    if (res.length > 0) {
      List<dynamic> list = res.toList().map((c) => User.fromMap(c)).toList();
